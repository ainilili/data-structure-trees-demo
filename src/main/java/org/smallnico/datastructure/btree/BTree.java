...
public class BTree<V> {
    ...
    protected void removeSingleNodeAdjust(BNode<V> rpg) {
        int pos = rpg.parentNode.indexOf(rpg.entries.getFirst().index);
        boolean isLeft = true;
        if(pos == -1) {
            pos = 0;
        }else {
            if(pos == rpg.parentNode.capacity() - 1) {
                isLeft = false;
            }
        }
        if(isLeft) {
            BEntry<V> bp = rpg.parentNode.entries.get(pos);
            BNode<V> br = bp.right;

            bp.right = br.entries.getFirst().left;
            br.entries.addFirst(bp);
            bp.group = br;

            if(pos > 0) {
                rpg.parentNode.entries.get(pos - 1).left = br;
            }
            rpg.parentNode.entries.remove(bp);
            if(rpg.parentNode.isBlank()) {
                this.root = br;
                br.parentNode = null;
            }
            insertEntryAdjust(br);
        }else {
            BNode<V> lpg = rpg;
            BEntry<V> bp = lpg.parentNode.entries.get(pos);
            BNode<V> bl = bp.left;

            bp.left = bl.entries.getLast().right;
            bl.entries.addLast(bp);
            bp.group = bl;

            if(pos > 0) {
                lpg.parentNode.entries.get(pos - 1).right = bl;
            }
            lpg.parentNode.entries.remove(bp);
            if(lpg.parentNode.isBlank()) {
                this.root = bl;
                bl.parentNode = null;
            }
            insertEntryAdjust(bl);
        }
